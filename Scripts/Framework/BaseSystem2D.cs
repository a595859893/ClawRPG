...
public abstract class BaseSystem2D : Node2D
{
    ...
    /// 单例实例 (子类需要实现)
    /// </summary>
    public static BaseSystem2D Instance { get; protected set; }
    ...
    {
        GD.Print($"[BaseSystem2D] {SystemName} initialized");
        IsInitialized = true;
    }
    ...
    public virtual void ImportSaveData(Godot.Collections.Dictionary data)
    {
        if (data == null) return;
    }
